...
- **结果视图**：主窗口和划词弹窗的翻译源标签右侧有「视图」菜单，可选单列列表或多列自适应；拉宽窗口自动增加列数，变窄自动回到单列，两处同步并记住选择
...
查看多个翻译源时，点击「视图 → 多列自适应」可利用横向空间并排阅读。卡片按翻译源顺序从左到右、从上到下排列；列数取决于可用宽度与字号，不会为了分列把译文挤得过窄。切换视图不重新请求翻译，单源结果仍使用完整宽度。旧配置默认保留单列列表。

开发验证：先构建自测工程，再运行自测 DLL 并传入 `--only-results` 可执行结果视图定向回归；加 `--shot` 可在 `shots/` 生成主窗口与弹窗的深浅主题预览，使用离线示例，不请求翻译服务。
