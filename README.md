...
框选之后点工具条上的「录制」（或按 `Ctrl+R`），那块区域开始录，`Esc` 或点浮条停下。到时长上限会自己停。录好的文件存进截图目录，提示条点一下能定位到它。

录的是**实时画面**，不是框选时那张定格图——画的标注不会进动图。帧率和时长上限在「设置 → 截图 → 录制动图」里，默认 10 fps、上限 30 秒。
